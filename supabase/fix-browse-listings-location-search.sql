-- Fix browse/search listing queries when Phase 5A/5B were not deployed yet.
-- Safe to re-run: adds missing columns and (re)creates the distance search RPC.
--
-- Run in Supabase SQL Editor if browse pages error on location_name / distance search.

-- Phase 5A columns (from google-maps-phase5a-listing-location.sql)
...
-- Phase 5B helpers (from google-maps-phase5b-distance-search.sql)

...
function GoogleIcon() {
  return (
    <svg className="auth-form__google-icon" viewBox="0 0 24 24" aria-hidden="true">
      <path
        fill="#4285F4"
...
function GoogleAuthButton({ postAuthRedirect = '/', disabled = false }) {
  ...
  async function handleClick() {
    ...
    const { error: oauthError } = await signInWithGoogle({ postAuthRedirect })
    ...
    // Browser navigates to Google on success; keep loading state until unload.
  }

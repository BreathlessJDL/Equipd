...
assert.match(appSrc, /AdminUserDetailPage/)
assert.match(appSrc, /users\/:userId/)
assert.match(appSrc, /AdminTrustSafetyPage/)
...
assert.match(dashboardSrc, /Marketplace activity/)
...
assert.equal(isAdminHubNavActive('/admin/trust-safety', ADMIN_HUB_NAV[2]), true)
assert.equal(isAdminHubNavActive('/admin/cases', ADMIN_HUB_NAV[0]), false)
assert.equal(isAdminHubNavActive('/admin/cases', ADMIN_HUB_NAV[3]), true)
assert.equal(isAdminHubNavActive('/admin/messages', ADMIN_HUB_NAV[6]), true)
assert.equal(isAdminHubNavActive('/admin/messages/abc', ADMIN_HUB_NAV[6]), true)
assert.equal(isAdminHubNavActive('/admin/intelligence/products', ADMIN_HUB_NAV[7]), true)
assert.equal(isAdminHubNavActive('/admin/intelligence/consoles', ADMIN_HUB_NAV[7]), true)
assert.equal(isAdminHubNavActive('/admin/catalogue', ADMIN_HUB_NAV[7]), true)
assert.equal(isAdminHubNavActive('/admin/price-guide/import', ADMIN_HUB_NAV[8]), true)
assert.equal(isAdminHubNavActive('/admin/cases', ADMIN_HUB_NAV[7]), false)

assert.equal(ADMIN_HUB_TOOLS.length, 8)
assert.equal(ADMIN_HUB_TOOLS[0].label, 'Users')
assert.equal(ADMIN_HUB_TOOLS[0].to, '/admin/users')
assert.equal(ADMIN_HUB_TOOLS[1].label, 'Trust & Safety')
assert.equal(ADMIN_HUB_TOOLS[5].label, 'Messages')
assert.equal(ADMIN_HUB_TOOLS[5].to, '/admin/messages')
for (const tool of ADMIN_HUB_TOOLS) {
  assert.ok(tool.to && tool.label && tool.description && tool.cta)
}

const adminRouteCount = [
  ...appSrc.matchAll(
    /path="(?:admin|users|trust-safety|cases|support|orders|catalogue|intelligence|price-guide)[^"]*"/g,
  ),
].length
assert.ok(adminRouteCount >= 18, `expected the existing admin route set, found ${adminRouteCount}`)

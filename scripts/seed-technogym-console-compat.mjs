#!/usr/bin/env node
/**
 * Seed curated Technogym console master + product_console_compat.
 * Replaces template backfill. Clears legacy product_console_options for the brand.
 *
 * Usage:
 *   node scripts/seed-technogym-console-compat.mjs
 *   node scripts/seed-technogym-console-compat.mjs --apply
 */
...
import {
  TECHNOGYM_BRAND,
  TECHNOGYM_CONSOLE_DEFS,
  TECHNOGYM_UNRESOLVED_PRODUCTS,
  buildTechnogymCompatByProductKey,
...
async function main() {
  ...
  console.log(`Technogym console seed (${args.dryRun ? 'dry-run' : 'APPLY'})`)
  console.log(`Consoles: ${TECHNOGYM_CONSOLE_DEFS.length}`)
  console.log('Unresolved notes:', TECHNOGYM_UNRESOLVED_PRODUCTS.map((entry) => entry.name).join('; '))

  const consoleRows = TECHNOGYM_CONSOLE_DEFS.map((def) => ({
    brand: TECHNOGYM_BRAND,
  ...
  const compatByKey = buildTechnogymCompatByProductKey(cardio)
  const productByKey = new Map((products ?? []).map((row) => [row.canonical_product_key, row]))
  const allIds = (products ?? []).map((row) => row.id)

  const unmapped = []
  for (const product of cardio) {
    if (compatByKey[product.canonical_product_key]) continue
    ...
    unmapped.push({
      ...
      reason: classified.reason || classified.kind,
      spin: isSpinBikeIndoorCycleProduct(product),
    })
  }

  console.log(`Approved: ${(products ?? []).length}; cardio: ${cardio.length}; mapped: ${Object.keys(compatByKey).length}; unmapped cardio: ${unmapped.length}`)
  for (const entry of unmapped.slice(0, 25)) {
    console.log(`  UNMAPPED ${entry.name} — ${entry.reason}`)
  }
  if (unmapped.length > 25) console.log(`  ... +${unmapped.length - 25} more`)
  ...
  if (!args.dryRun) {
    if (allIds.length) {
      ...
      console.log(`Cleared compat + legacy options for ${allIds.length} Technogym products`)
    }

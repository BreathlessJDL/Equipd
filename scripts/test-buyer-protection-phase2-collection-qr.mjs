#!/usr/bin/env node
/**
 * End-to-end verification for Buyer Protection Phase 2 (Collection QR).
 *
 * Usage:
 *   node scripts/test-buyer-protection-phase2-collection-qr.mjs
 *
...
async function main() {
  ...
  logStep('Locate paid awaiting_collection order')
  ...
  if (!order) {
    throw new Error(
      'No paid awaiting_collection order found. Run scripts/test-buyer-protection-phase1.mjs first.',
    )
  }

  logPass(`Using order ${order.id}`)

  logStep('Seller generates collection QR token')
  ...
  logPass(`Token generated, expires ${tokenData.expires_at}`)

  logStep('Wrong buyer cannot confirm collection')
  ...
  assert(sellerConfirmError, 'Seller confirmation should fail')
  logPass(`Seller rejected: ${sellerConfirmError.message}`)
  ...
  logPass('Buyer preview ready with payment summary fields')
  ...
  assert(confirmedOrder.fulfilment_status === 'collected', 'Expected collected status')
  ...
  assert(confirmedOrder.payout_status === 'not_due', 'Payout should remain not_due')
  assert(confirmedOrder.payout_release_at, 'Expected payout_release_at')
  assert(!confirmedOrder.stripe_transfer_id, 'No Stripe transfer should exist')
  ...
  logPass('Buyer confirmed collection with 24-hour payout hold')

  logStep('Verify notifications created')
  const { data: notifications } = await admin
    .from('notifications')
    .select('user_id, type, title')
    .eq('type', 'collection_confirmed')
    .in('user_id', [BUYER.id, SELLER.id])
    .order('created_at', { ascending: false })
    .limit(4)

  assert((notifications ?? []).length >= 2, 'Expected buyer and seller notifications')
  logPass('Collection confirmed notifications created')

  console.log('\nAll Buyer Protection Phase 2 collection QR checks passed.')

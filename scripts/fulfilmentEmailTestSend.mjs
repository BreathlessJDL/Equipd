...
function assertSubjectPopulated(dynamicData, templateKey) {
  ...
  if (!subject) {
    return {
      ok: false,
      error: `Subject is empty for template "${templateKey}". Fulfilment test sends require a populated subject.`,
    }
  }
...
function printSendSummary({ templateKey, templateId, subject, recipient, sendGridResponse, dryRun }) {
  console.log('')
  console.log('--- Fulfilment email test send ---')
...
/**
 * Send one fulfilment template to a recipient using mock compose data only.
 * No marketplace tables, orders, or edge functions are involved.
 */
export async function sendFulfilmentTestEmail({
  templateKey,
  to,
  getEnv = (key) => process.env[key] ?? '',
}) {
  if (!isFulfilmentEmailTemplateKey(templateKey)) {
    return { ok: false, error: `Unknown fulfilment template key: ${templateKey}` }
  }
  ...
  const dynamicData = buildFulfilmentTestDynamicData(templateKey, getEnv)
  if (!dynamicData) {
...
export async function sendAllFulfilmentTestEmails({
  to,
  getEnv = (key) => process.env[key] ?? '',
}) {
  const results = []
  ...
  for (const templateKey of FULFILMENT_EMAIL_TEMPLATE_KEYS) {
    const result = await sendFulfilmentTestEmail({ templateKey, to, getEnv })
    results.push({ templateKey, ...result })
    if (!result.ok) {
      return { ok: false, failedAt: templateKey, results }
    }
  }

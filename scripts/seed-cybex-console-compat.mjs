#!/usr/bin/env node
/**
 * Seed Cybex console master + product_console_compat mappings.
 *
 * Usage:
 *   node scripts/seed-cybex-console-compat.mjs
 *   node scripts/seed-cybex-console-compat.mjs --apply
 */
...
import {
  CYBEX_BRAND,
  CYBEX_COMPAT_BY_PRODUCT_KEY,
  CYBEX_CONSOLE_DEFS,
  CYBEX_UNRESOLVED_PRODUCTS,
} from '../src/lib/cybexConsoleCompat.js'
...
async function main() {
  ...
  const productKeys = Object.keys(CYBEX_COMPAT_BY_PRODUCT_KEY)

  console.log(`Cybex console seed (${args.dryRun ? 'dry-run' : 'APPLY'})`)
  console.log(`Consoles: ${CYBEX_CONSOLE_DEFS.length}`)
  console.log(`Mapped product keys: ${productKeys.length}`)
  console.log('Unresolved:', CYBEX_UNRESOLVED_PRODUCTS.map((entry) => entry.name).join('; '))

  const consoleRows = CYBEX_CONSOLE_DEFS.map((def) => ({
    brand: CYBEX_BRAND,
  ...
  const unmappedCardio = cardio.filter((row) => !CYBEX_COMPAT_BY_PRODUCT_KEY[row.canonical_product_key])

  if (missingKeys.length) console.warn('Mapped keys not in DB:', missingKeys.join(', '))
  if (unmappedCardio.length) {
    console.warn('Approved Cybex cardio without mappings:', unmappedCardio.map((row) => row.canonical_product_key).join(', '))
  }
  console.log(`Approved Cybex cardio: ${cardio.length}; strength/other left unmapped: ${(products ?? []).length - cardio.length}`)

  const compatRows = []
  for (const [productKey, mappings] of Object.entries(CYBEX_COMPAT_BY_PRODUCT_KEY)) {
    const product = productByKey.get(productKey)
    if (!product) continue
    if (!isCardioEquipmentProduct(product)) {
      console.warn(`Refusing to map non-cardio product: ${productKey}`)
      continue
    }

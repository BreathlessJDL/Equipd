#!/usr/bin/env node
/**
 * Technogym image backfill dry-run with strict line matching.
 *
 * Usage:
 *   node scripts/dry-run-technogym-product-images.mjs
 *   node scripts/dry-run-technogym-product-images.mjs --limit 50
 */
...
async function fetchTechnogymProducts(supabase) {
...
async function main() {
  ...
  const products = await fetchTechnogymProducts(supabase)
  const missing = products.filter(productIsMissingImage)
  ...
  const summary = {
    technogymProducts: products.length,
  ...
  console.log('Technogym image dry-run (no database writes)')
  console.log('')
  console.log(`Technogym approved products: ${summary.technogymProducts}`)
  ...
  for (const sample of samples) {
    ...
    console.log(`Confidence: ${sample.confidence} (${sample.confidenceBand})`)
    console.log(`Product line: ${sample.productLine}`)
